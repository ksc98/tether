...
int main(int argc, char** argv) {
    ...
    if (wayland_srv.init()) {
        wayland_srv.set_clipboard_callback([](const std::string& text) {
            ...
            // write is what reaches it in the background.
    ...
    // Writes desktop clipboard changes into the characteristic the phone app serves.
    std::unique_ptr<tether::bluetooth::ClipboardGattClient> clipboard_gatt;
    ...
    if (bluez.start()) {
        ...
        clipboard_gatt = std::make_unique<tether::bluetooth::ClipboardGattClient>(bluez);
